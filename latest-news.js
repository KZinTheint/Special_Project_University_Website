      ...
      .then(data => {
        if (data.success && data.data.news.length > 0) {
          ...
          const allNewsHtml = allNews.map(newsItem => {
            // Get description from content, handle null values properly
            let description = '';
            if (newsItem.content && newsItem.content.length > 0 && newsItem.content[0].description) {
              description = newsItem.content[0].description;
            }
